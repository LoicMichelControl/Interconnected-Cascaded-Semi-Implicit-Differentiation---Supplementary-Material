    ...
    if ( cascade_diff == 1)
        ...
        fprintf(fid,'%d %1.20f & %1.20f & %1.20f & %1.20f & %1.20f & %1.20f \n', caseNumber, error(1,1), error(1,2), error(2,1), error(2,2), error(3,1), error(3,2));
        fclose all;
    
        % standard
        fid = fopen('Result_error_evaluation.txt', 'a');
        fprintf(fid, '\n ******* %s \n\n', name);
        fprintf(fid, ['[ avg(e_1)_(last 1/4) = %1.2e, max( |e_1|) = %1.3f ] \n' ...
